...
interface TotalTriathlonsTabProps {
  events: AggregatedEventData[];
}

export default function TotalTriathlonsTab({ events }: TotalTriathlonsTabProps) {
  // Filtrer uniquement les triathlons et calculer les totaux par année
  const yearlyTotals = useMemo(() => {
    const triathlonEvents = events.filter(e => e.isTriathlon);
  ...
  return (
    <div className="space-y-6">
      {/* Titre */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Total Triathlons / Swimrun / T24 / Ventouxman / Bayman</h2>
        <p className="text-gray-600 mt-1">Vue d'ensemble de tous les triathlons et épreuves similaires par année</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Évolution du CA Triathlons</h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="bg-yellow-500 px-6 py-4 text-center">
          <h3 className="text-xl font-bold text-gray-900">Total Triathlons / Swimrun / T24 / Ventouxman / Bayman</h3>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {yearlyTotals.totals.map((yearData, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    {yearData.runners > 0 ? yearData.runners.toLocaleString('fr-FR') : '-'}
                  </td>
                  <td className="px-4 py-3 text-center">
                    {yearData.revenue > 0 ? `€${yearData.revenue.toLocaleString('fr-FR')}` : '-'}
                  </td>
                  <td className="px-4 py-3 text-center">
                    {yearData.buyers > 0 ? yearData.buyers.toLocaleString('fr-FR') : '-'}
                  </td>
                  <td className="px-4 py-3 text-center">
                    {yearData.revenuePerRunner > 0 ? `€${yearData.revenuePerRunner.toFixed(2)}` : '-'}
                  </td>
                  <td className="px-4 py-3 text-center">
              ...
              {/* Ligne Total */}
              <tr className="bg-yellow-500 text-gray-900 font-bold">
